...
import { Trash2, Plus, GraduationCap, Users, Clock } from "lucide-react";
...
interface ClassItem {
  ...
  grade: string;
  maxStudents: number;
  hoursPerWeek: number; // Ώρες/εβδομάδα ανά μάθημα (για το αυτόματο πρόγραμμα)
}
...
export default function ClassesPage() {
  ...
  const [hoursPerWeek, setHoursPerWeek] = useState<string>("2");
  ...
  useEffect(() => {
    ...
    const normalized = rawClasses.map((c: any) => ({
      ...
      hoursPerWeek: Number(c.hoursPerWeek) > 0 ? Number(c.hoursPerWeek) : 1, // default 1 αν δεν υπήρχε
    }));
  ...
  const addClass = () => {
    ...
    if (!maxStudents || isNaN(parsedMax) || parsedMax <= 0) {
      alert("Παρακαλώ όρισε έναν έγκυρο μέγιστο αριθμό ατόμων (π.χ. 15).");
      return;
    }

    const parsedHours = parseInt(hoursPerWeek);
    if (!hoursPerWeek || isNaN(parsedHours) || parsedHours <= 0) {
      alert("Παρακαλώ όρισε έγκυρες ώρες/εβδομάδα (π.χ. 2).");
      return;
    }
  ...
  return (
    <WorkspaceShell title="Διαχείριση Τμημάτων" description="Ορίστε τα τμήματα ανά τάξη, τον μέγιστο αριθμό μαθητών και τις ώρες/εβδομάδα.">
      ...
      <div className="bg-[#1e2330] p-6 rounded-3xl border border-slate-800 mb-8 max-w-2xl mx-auto shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1.5 pl-1">Ώρες / Εβδομάδα *</label>
              <input
                type="number"
                min="1"
                max="10"
            ...
            <div key={c.id} className="bg-[#1e2330] border border-slate-800 p-4 rounded-2xl flex justify-between items-start hover:border-slate-700 transition-all">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 pt-0.5">
                  ...
                  <span className="text-amber-400 text-[10px] font-bold bg-amber-950/40 px-2 py-0.5 rounded border border-amber-900/30 flex items-center gap-1">
                    <Clock size={10}/> {c.hoursPerWeek} ώρες/εβδ.
                  </span>
                </div>
              </div>
