...
export default function ClassesPage() {
  ...
  const addClass = () => {
    if (!className.trim()) { alert("Παρακαλώ συμπλήρωσε το όνομα του τμήματος."); return; }
    if (!grade) { alert("Παρακαλώ επιλέξτε την Τάξη στην οποία ανήκει το τμήμα."); return; }
    const parsedMax = parseInt(maxStudents);
    if (!maxStudents || isNaN(parsedMax) || parsedMax <= 0) { alert("Παρακαλώ όρισε έγκυρο μέγιστο αριθμό ατόμων (π.χ. 15)."); return; }
    ...
    setClassName(""); setGrade(""); setMaxStudents("");
  };
  ...
  return (
    <WorkspaceShell title="Διαχείριση Τμημάτων" description="Ορίστε τα τμήματα ανά τάξη και τον μέγιστο αριθμό μαθητών. (Οι ώρες ορίζονται ανά μάθημα στη σελίδα Μαθημάτων.)">
