...
import { Plus, Trash2, BookOpen, AlertTriangle, UserCheck, Users, Clock, CalendarDays, Maximize2, GitBranch, X, CheckCircle2 } from "lucide-react";
...
export default function LessonsPage() {
    ...
    if (raw) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0 && typeof parsed[0] === 'string') {
          const migrated: Lesson[] = parsed.map((n: string) => ({
  ...
  const addLesson = () => {
    ...
    if (distribution.length === 0) { alert("Πρέπει να δηλώσεις κατανομή ωρών."); return; }
    if (currentSum !== weeklyHours) { alert(`Η κατανομή (${currentSum}) δεν αντιστοιχεί στις συνολικές ώρες (${weeklyHours}).`); return; }
    ...
    const newLesson: Lesson = {
      ...
      maxHoursPerDay: 2, // Hardcoded locked
      minGapDays: minGap
    };
      ...
      <div className="bg-[#1e2330] p-6 rounded-3xl border border-slate-800 mb-8 max-w-4xl mx-auto shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" className="w-full bg-[#0b0e14] border border-slate-800 p-3 rounded-xl text-xs text-white" value={minGap} onChange={e => setMinGap(Number(e.target.value))} />
          </div>
          ...
          <div className="col-span-full">
            ...
            <div className={`mt-3 p-3 rounded-xl border text-xs font-bold flex items-center justify-between ${currentSum === weeklyHours ? "bg-emerald-950/20 border-emerald-900/50 text-emerald-400" : "bg-slate-900 border-slate-800 text-slate-400"}`}>
                <div className="flex items-center gap-2">
                    {currentSum === weeklyHours ? <CheckCircle2 size={14}/> : <Clock size={14}/>}
                    Κατανομή: {distribution.length > 0 ? distribution.join('h + ') + 'h' : '0h'} 
                    <span className="opacity-70 ml-1">= {currentSum}/{weeklyHours} ώρες</span>
                </div>
                <div>
                    {currentSum === weeklyHours 
                        ? "✅ Ολοκληρώθηκε" 
                        : currentSum < weeklyHours 
                            ? `Υπολείπεται ${weeklyHours - currentSum}h`
                            : `Υπέρβαση ${currentSum - weeklyHours}h ❌`
                    }
                </div>
            </div>
          </div>
              ...
              <div key={lesson.id} className="bg-[#1e2330] border border-slate-800 p-4 rounded-2xl flex flex-col justify-between gap-4 hover:border-slate-700 transition-all">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4 text-[10px] text-slate-400">
                    <div className="flex items-center gap-1.5"><Clock size={12}/> {lesson.weeklyHours} ώρες/εβδ</div>
                    <div className="flex items-center gap-1.5"><CalendarDays size={12}/> {lesson.distribution.join('-')}</div>
                    <div className="flex items-center gap-1.5"><Maximize2 size={12}/> Max 2h/day</div>
