...
export async function fetchStudents(): Promise<Student[]> {
  ...
  const { data, error } = await supabase.from<Student>("students").select("*").order("fullName");
...
export async function createStudent(student: Student): Promise<Student> {
  ...
  const { data, error } = await supabase.from<Student>("students").insert(student).select().single();
...
export async function updateStudent(id: string, updates: Partial<Student>): Promise<Student | null> {
  ...
  const { data, error } = await supabase.from<Student>("students").update(updates).eq("id", id).select().single();
...
export async function fetchTeachers(): Promise<Teacher[]> {
  ...
  const { data, error } = await supabase.from<Teacher>("teachers").select("*").order("fullName");
...
export async function createTeacher(teacher: Teacher): Promise<Teacher> {
  ...
  const { data, error } = await supabase.from<Teacher>("teachers").insert(teacher).select().single();
...
export async function updateTeacher(id: string, updates: Partial<Teacher>): Promise<Teacher | null> {
  ...
  const { data, error } = await supabase.from<Teacher>("teachers").update(updates).eq("id", id).select().single();
  return error || !data ? null : data;
}
...
export async function deleteTeacher(id: string): Promise<boolean> {
  ...
  const { error } = await supabase.from<Teacher>("teachers").delete().eq("id", id);
  return !error;
}
...
export async function fetchCourses(): Promise<Course[]> {
  ...
  const { data, error } = await supabase.from<Course>("courses").select("*").order("title");
...
export async function createCourse(course: Course): Promise<Course> {
  ...
  const { data, error } = await supabase.from<Course>("courses").insert(course).select().single();
...
export async function updateCourse(id: string, updates: Partial<Course>): Promise<Course | null> {
  ...
  const { data, error } = await supabase.from<Course>("courses").update(updates).eq("id", id).select().single();
  return error || !data ? null : data;
}
...
export async function deleteCourse(id: string): Promise<boolean> {
  ...
  const { error } = await supabase.from<Course>("courses").delete().eq("id", id);
  return !error;
}
...
export async function fetchClassrooms(): Promise<Classroom[]> {
  ...
  const { data, error } = await supabase.from<Classroom>("classrooms").select("*").order("name");
...
export async function saveSchedule(slots: ScheduleSlot[]): Promise<ScheduleSlot[]> {
  ...
  const { error: deleteError } = await supabase.from<ScheduleSlot>("schedules").delete().neq("id", "");
  if (deleteError) {
    throw deleteError;
  }

  const { data, error } = await supabase.from<ScheduleSlot>("schedules").insert(slots).select();
  if (error || !data) {
