...
export default function TeachersPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !specialty) return alert("⚠️ Συμπλήρωσε Όνομα και Ειδικότητα!");
    ...
    setName(""); setSpecialty(""); setAvailability({});
  };
  ...
  return (
    <WorkspaceShell title="Διαχείριση Καθηγητών" description="Όρισε καθηγητές και το μάθημα που διδάσκουν.">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-4">
        
        {/* ΦΟΡΜΑ */}
        <div className="bg-[#1e2330] border border-slate-800 p-6 rounded-3xl h-fit">
          <form onSubmit={handleSave} className="space-y-4">
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ονοματεπώνυμο Καθηγητή" className="w-full bg-[#0b0e14] border border-slate-800 p-2 rounded text-xs text-white" />
            <input type="text" value={specialty} onChange={e => setSpecialty(e.target.value)} placeholder="Ειδικότητα (π.χ. Μαθηματικά)" className="w-full bg-[#0b0e14] border border-slate-800 p-2 rounded text-xs text-white" />
            
            <div className="space-y-2 pt-2 border-t border-slate-800">
              <p className="text-[10px] font-bold text-slate-400">Διαθεσιμότητα Καθηγητή</p>
            ...
            {teachers.map(t => (
              <div key={t.id} className="bg-[#0b0e14] p-3 rounded border border-slate-800 flex justify-between items-center">
                <div className="text-xs">
                  ...
                  <p className="text-purple-400 text-[10px]">Ειδικότητα: {t.specialty}</p>
                </div>
