...
type Teacher = {
  ...
  phone?: string;
  email?: string;
  subject?: string;       // legacy single — διατηρείται για compat
  subjects?: string[];    // ΝΕΟ: πολλαπλά μαθήματα
  preferredClasses?: string[]; // ονόματα τμημάτων, π.χ. ["Γ1", "Γ2"]
...
export default function TeachersPage() {
  ...
  useEffect(() => {
    // Load teachers με auto-migration
    ...
    if (storedT) {
      try {
        ...
        const migrated = parsed.map((t: any) => ({
          ...t,
          // Migration: αν υπάρχει μόνο subject (legacy), φτιάχνω subjects array
    ...
    // Load courses (από οποιοδήποτε key)
    ...
    // Load classes (με auto-migration category→grade)
  ...
  const handleSave = () => {
    ...
    const teacher: Teacher = {
      ...
      subject: form.subjects?.[0] || "", // legacy compat: ο scheduler κοιτάει subject (string)
  ...
  const toggleSubject = (subjName: string) => {
    ...
    setForm({
      ...form,
      subjects: current.includes(subjName) ? current.filter((s) => s !== subjName) : [...current, subjName],
    });
  };

  const togglePreferredClass = (clsName: string) => {
    const current = form.preferredClasses || [];
    setForm({
      ...form,
      preferredClasses: current.includes(clsName) ? current.filter((c) => c !== clsName) : [...current, clsName],
    });
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Στοιχεία, μαθήματα και τμήματα προτίμησης.</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {/* ===== ΜΑΘΗΜΑΤΑ ΔΙΔΑΣΚΑΛΙΑΣ — ΠΟΛΛΑΠΛΗ ΕΠΙΛΟΓΗ ===== */}
                ...
                <div className="mt-2 bg-zinc-800 border border-indigo-500 rounded-lg p-3 max-h-80 overflow-y-auto">
                  {courses.length === 0 ? (
                    <p className="text-amber-400 text-xs text-center py-4">
                      ⚠️ Δεν υπάρχουν μαθήματα. Πήγαινε στη σελίδα «Μαθήματα» και δημιούργησε πρώτα τα μαθήματα.
                    </p>
                  ) : (
                      ...
                      <div key={grade} className="mb-3 last:mb-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
                          {coursesByGrade[grade].map((c) => (
                            <label key={c.name}
                              className="flex items-center gap-2 text-sm cursor-pointer hover:bg-zinc-700 px-2 py-1 rounded">
                              <input type="checkbox"
            ...
            {/* ===== ΤΜΗΜΑΤΑ ΠΡΟΤΙΜΗΣΗΣ (από τα classes) ===== */}
            <div className="md:col-span-2">
              <label className="text-xs text-zinc-400 font-semibold uppercase tracking-wide mb-1 block">
              ...
              <button type="button" onClick={() => setShowClassesDropdown(!showClassesDropdown)}
                ...
                <span className="text-sm">
                  {form.preferredClasses && form.preferredClasses.length > 0
                    ? `✓ ${form.preferredClasses.length} τμήματα: ${form.preferredClasses.slice(0, 3).join(", ")}${form.preferredClasses.length > 3 ? "..." : ""}`
              ...
              {showClassesDropdown && (
                <div className="mt-2 bg-zinc-800 border border-indigo-500 rounded-lg p-3 max-h-80 overflow-y-auto">
                  {classes.length === 0 ? (
                    <p className="text-amber-400 text-xs text-center py-4">
                      ⚠️ Δεν υπάρχουν τμήματα. Πήγαινε στη σελίδα «Τμήματα» και δημιούργησε πρώτα.
                    </p>
                  ) : (
          ...
          <div className="mt-5 pt-5 border-t border-zinc-700">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs text-zinc-400">(Δευ-Παρ 09:00-14:00)</span>
            </label>
          </div>
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-zinc-500 mt-1 flex items-center gap-3 flex-wrap">
                ...
                {t.preferredClasses && t.preferredClasses.length > 0 && (
                  <span className="text-indigo-400">📌 {t.preferredClasses.length} τμήματα: {t.preferredClasses.slice(0, 4).join(", ")}{t.preferredClasses.length > 4 ? "..." : ""}</span>
                )}
